...
if (Test-Path NVEncC) {
    ...
    if (Test-Path ..\pkg\$nvencBin) {
        ...
        if ($nvencBinSHA256 -and ((Get-FileHash ..\pkg\$nvencBin -Algorithm SHA256).Hash -ne $nvencBinSHA256)) {
            throw "Hash error: $nvencBin"
        }
        7z.exe e -oNVEncC ..\pkg\$nvencBin */*.txt */*.url */NVEncC/*.url */NVEncC/x64/*
        pushd NVEncC
...
if (Test-Path QSVEncC) {
    ...
    if (Test-Path ..\pkg\$qsvencBin) {
        ...
        7z.exe e -oQSVEncC ..\pkg\$qsvencBin */*.bat */*.rtf */*.txt */*.url */QSVEncC/*.url */QSVEncC/x64/*
        pushd QSVEncC
...
if (Test-Path VCEEncC) {
    ...
    if (Test-Path ..\pkg\$vceencBin) {
        ...
        7z.exe e -oVCEEncC ..\pkg\$vceencBin */*.txt */*.url */VCEEncC/x64/*
        pushd VCEEncC
