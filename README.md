...
[このブランチ](https://github.com/xtne6f/TVRemotePlus/tree/master) はおもに自分用の改変と、EDCB サイドからの情報提供を目的としています。  
この README の編集は基本的に行わないので、変更点は各コミットの履歴を参照してください。  
もし利用したいコミットがあれば、自由にコピペや cherry-pick してください。  
オリジナルと異なり、`bin/` フォルダは `pkg/` フォルダの各パッケージから生成します。最初に `prepare-bin.bat` を実行してください。  
(追記終わり)
